            ...
            <div class="card-body">
              <div class="form-group">
                ...
                <select class="form-control custom-select" name="parent_id">
                  ...
                  @if(!empty($cat))
                    @foreach($cat as $list)
                      <option value="{{ $list->id }}" @if($list->id == $select_cat->id) selected @endif>{{ $list->name }}</option>
                      @php
                        $child_cat = App\ProductCategory::where('status',1)->where('parent_id',$list->id)->get();
                      @endphp
                        @foreach($child_cat as $child)
                         <option value="{{ $child->id }}" @if($child->id == $select_cat->id) selected @endif>--{{ $child->name }}</option>
                        @endforeach
                    @endforeach
                  @endif
                </select>
          ...
          <div class="card card-secondary">
            ...
            <div class="card-body">
              ...
              <div class="form-group">
                ...
                <textarea name="meta_descrption" id="inputSpentBudget" class="form-control" value="{{ $Data->meta_descrption }}"></textarea>
              </div>
